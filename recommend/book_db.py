...
def query(title: str, author: str) -> List[Work]:
    ...
    for book in db_entries:
        ...
        composite = goog.composite_ratio(title_ratio, author_ratio)
    ...
    if not filtered:
        print("Didn't find in the local database; looking up on the Google API")
        api_book = save_from_api(title, author, db_entries)
        if not api_book:
            return []
        return api_book
